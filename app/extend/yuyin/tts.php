...
function baidu_tts($content,$file){
    # 填写网页上申请的appkey 如 $apiKey="g8eBUMSokVB1BHGmgxxxxxx"
    $apiKey = "5OKkG35LEqv15HxdGFKbgf1Q";
    # 填写网页上申请的APP SECRET 如 $secretKey="94dc99566550d87f8fa8ece112xxxxx"
    $secretKey = "KcGnkSFMDLaX66Bq8maVL7u6eOQM2Htd";
    ...
    $text2 = mb_convert_encoding($text,"GBK","UTF-8");
    //echo "text length :" . mb_strlen($text2, "GBK") . "\n";die;
    $_data['text_length'] =  "text length :" . mb_strlen($text2, "GBK") . "\n";
    ...
    if (!isset($response['access_token'])){
        ...
        $_data['TOKEN'] = "ERROR TO OBTAIN TOKEN\n";
        exit(1);
    }
    if (!isset($response['scope'])){
        //echo "ERROR TO OBTAIN scopes\n";
        $_data['scopes'] = "ERROR TO OBTAIN scopes\n";
        exit(2);
    }
    if (!in_array('audio_tts_post',explode(" ", $response['scope']))){
        //echo "DO NOT have tts permission\n";
        $_data['permission'] = "DO NOT have tts permission\n";
        // 请至网页上应用内开通语音合成权限
        exit(3);
    }
    ...
    {
        //echo curl_error($ch);
        $_data['curl_error'] = curl_error($ch);
        exit(2);
    }
    curl_close($ch);
